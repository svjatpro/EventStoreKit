...
public static class DbExtensions
{
    ...
    {
        Run(factory.CreateDbProvider, action, isolationLevel, processException, transaction);
    }
    public static void Run(
        this Func<IDbProvider> factory,
        Action<IDbProvider> action,
        IsolationLevel isolationLevel = IsolationLevel.ReadCommitted,
        Action<Exception>? processException = null,
        bool transaction = true)
    {
        using var dbProvider = factory();
        try
        {
            if (transaction)
            {
    ...
    public static TResult Run<TResult>(
        ...
        Action<Exception>? processException = null)
    {
        return Run(factory.CreateDbProvider, action, isolationLevel, processException);
    }
    public static TResult Run<TResult>(
        this Func<IDbProvider> factory,
        Func<IDbProvider, TResult> action,
