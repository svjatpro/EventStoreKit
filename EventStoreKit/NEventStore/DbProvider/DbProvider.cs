...
namespace EventStoreKit.NEventStore.DbProvider
{
    public class DbProvider(DataConnection dbConnection) : IDbProvider
    {
        ...
        public bool CreateTable<T>(bool overwrite = false) where T : class
        {
            ...
            var table = dbConnection.GetTable<T>();

            // When [Table] has no explicit Schema, linq2db leaves SchemaName null,
            // but the DB row carries the real schema (current_schema() on the
            // connection's SearchPath). Resolve it so the existence check matches.
            var effectiveSchema = table.SchemaName ?? GetCurrentSchema();
            var tableExist = dbSchema.Tables.Any(t =>
                t.SchemaName == effectiveSchema &&
        ...
        public int Insert<T>(T entity) where T : class
        {
            ...
            //Console.WriteLine(dbConnection.LastQuery);
            return result;
        }
        ...
        public int InsertOrUpdate<T>(
            ...
            where T : class
        {
            return dbConnection.GetTable<T>().InsertOrUpdate(predicate, evaluator);
        }
        ...
        public int Update<T>(
            ...
            where T : class
        {
            var result = dbConnection.GetTable<T>().Update(predicate, evaluator);
            //Console.WriteLine(dbConnection.LastQuery);
            return result;
        }
